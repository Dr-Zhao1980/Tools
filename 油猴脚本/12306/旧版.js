...
(function() {
    ...
    const UIModule = (() => {
        ...
        return {
            init: (startCb, stopCb) => { createPanel(); onStartCallback = startCb; onStopCallback = stopCb; log('抢票助手 UI 已初始化', 'success'); log('已自动加载上次配置', 'info'); },
            log: log,
    ...
    function calculatePollingDelay(config) {
        ...
        if (config.startTime) {
            ...
            if (diff > 10000) {
                ...
                // 极速模式：开售前 500ms ~ 开售后 2s 内，每 50ms 查一次 (关键时刻)
                delay = 50;
            } else {
